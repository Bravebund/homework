...
public class LgDispatcherServlet extends HttpServlet {
     ...
     * 获取每一个Handler的授权数据
     */
    private void doHandlerSecurity() {
        ...
        for (Handler handler : handlerMapping) {
            ...
            if (aClass.isAnnotationPresent(Security.class)) {
                ...
                securityList.addAll(Arrays.asList(value1));
            }
            ...
            if (method.isAnnotationPresent(Security.class)) {
                ...
                securityList.addAll(Arrays.asList(value2));
            }
            List<String> distinctList = securityList.stream().distinct().collect(Collectors.toList());
            String securityStr = StringUtils.join(distinctList, ",");
            handler.setSecurityStr(securityStr);
        }
    }
...
//        method.invoke() //
        ...
        // 权限限定
        String securityStr = handler.getSecurityStr();
        if (securityStr != null && securityStr.trim().length() > 0) {
            // 有限定值时，需要判定当前请求是否有权限访问
            ...
            if (usernames != null) {
                for (int i = 0; i < usernames.length; i++) {
                    ...
                    if (securityStr.contains(username)){
                        flag = true;
                        break;
                    }
                }
            ...
            if (!flag){
                resp.getWriter().write("403 permission denied");
                return;
            }
        }
